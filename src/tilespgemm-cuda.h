    ...
    for (int tilei = tile_start; tilei < tile_end; tilei++)
    {
        ...
        if (tilei < numblkC)
        {
            ...
            lenb = bstop - bbase;
            {
                ...
                // g16.sync();
                // this works
                // matchedcnt = g16.shfl(matchedcnt, 0);
    ...
    gettimeofday(&t1, NULL);
        ...
        *time_malloc += (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
        ...
        if (blksmem_tny_cnt)
        {
            ...
            tile_spgemm_step4_cuda_kernel_smem_v3<SMEM_TNY_TH><<<num_blocks, num_threads, 0, streams[0]>>>(d_blkrowptrA, d_blkcolidxA, d_nnzb_A, d_blkcsr_Val_A, d_blkcsr_Col_A, d_blkcsr_Ptr_A,
                                                                                                                    blkmA, blknA, numblkA, nnzA,
